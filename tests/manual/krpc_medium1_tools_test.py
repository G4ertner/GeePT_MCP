...
def main() -> int:
    ap = argparse.ArgumentParser(description="Test medium batch 1 tools: engines/resources/surface")
    ap.add_argument("--address", required=True)
    ap.add_argument("--rpc-port", type=int, default=50000)
    ap.add_argument("--stream-port", type=int, default=50001)
    ap.add_argument("--name", default="Medium1 Test")
    args = ap.parse_args()
    ...
    tests = [
        ("engine_status", readers.engine_status),
        ("resource_breakdown", readers.resource_breakdown),
        ("surface_info", readers.surface_info),
    ]
